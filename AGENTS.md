...
* **Target Identification:** Drives are routed by whether they contain a `packfiles.txt` marker on their root. Drives **with** `packfiles.txt` get the full build via `process_drive()`. Drives **without** it are treated as "empty" drives and handed to `process_empty_drive()` (they are no longer skipped). Both paths report a completed drive to the WebUI and history.
* **Empty-Drive Workflow (`process_empty_drive`):** Copies only the regional base files. Because there is no `packfiles.txt` to read the region from, the region is inferred by the LLM from the **drive's name** (`classifyRegionByName`, which pulls the name via `get_volume_name` and feeds it to the LLM running on the drive root). The LLM records its answer as `region-U.txt` / `region-I.txt` on the root (contents = the drive name). `copy_base_files` (a thin wrapper over `copy_region_files`) then copies the base set. Finally **both** `dobDir` **and** the LLM-created region file are deleted, and the drive is reported completed (success).
* **WebUI Status Numbers:** The full build uses steps 1–7; the empty-drive path uses step 8 ("Identifying Region" from the name) and step 9 ("Copying Base Files"). Steps 1–9 are all "processing" (see `isProcessing` in `app.js`); 0 is idle and 10/11 are terminal. `copy_region_files` accepts a `status_number` arg so the same copy logic can report step 3 (full build) or step 9 (empty drive).
