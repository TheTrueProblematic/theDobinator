...
* **Unplugged-drive handling:** On drive removal the monitor loop drops the drive from both the blank-awaiting and pending lists; a job already in the work queue is additionally skipped by the worker (`os.path.exists` guard) so it never tries to build a drive that's gone.
